...
// GET /cuentas/credito/{cod}/observaciones -> ObservacionCreditoOut[]
export async function getObservacionesCredito(codcuentacredito) {
  const { data } = await hbApi.get(
    `/cuentas/credito/${encodeURIComponent(codcuentacredito)}/observaciones`,
  )
  return Array.isArray(data) ? data : []
}
